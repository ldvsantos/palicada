---
title: "Controle de erosão com a implantação de paliçadas em área de ocorrência de Plintossolo degradado pela presença de ravinas"
author: Emersson Guedes da Silva et al.
...
As práticas de conservação do solo visam restabelecer o equilíbrio hidrológico e controlar a energia do escoamento. No contexto de erosão linear, as intervenções estruturais buscam reduzir a velocidade do fluxo e estabilizar o perfil longitudinal do canal. A bioengenharia de solos oferece soluções de engenharia hidráulica baseadas em materiais naturais para a dissipação de energia e estabilização de canais. As paliçadas (*check dams*) atuam como barreiras permeáveis que alteram a geometria hidráulica do canal, reduzindo a declividade efetiva e a velocidade do fluxo, o que diminui a capacidade de transporte de sedimentos e induz a deposição a montante [@guerra_bezerra_jorge_2023].
...
![Figura 1 -- Área de ocorrência de ravinas, localizada no Município de São Cristóvão - Sergipe](media/artigo/image1.jpg){width="6.188623140857393in"
...
![Figura 2 -- Foto do perfil do solo classificado como PLINTOSSOLO Argilúvico distrófico](media/artigo/image11.png){width="2.494268372703412in" height="3.2583333333333333in"}


Os atributos químicos e físicos obtidos para os horizontes descritos (Tabela 1) fundamentam a interpretação da impedância hidrológica do perfil e da sua resposta ao escoamento concentrado.
...
![Figura 3 -- Ocorrência de ravinas na área estudada](media/artigo/Fig_3v2.png){width="6.5in"}
...
As toras de bambu foram dispostas horizontal e verticalmente, parcialmente enterradas para aumentar a resistência às enxurradas. Optou-se por adicionar 30 cm extras ao comprimento de cada tora, possibilitando o soterramento de 15 cm em cada lado da ravina (nos taludes) e de 30 cm nas toras verticais, garantindo maior estabilidade estrutural.

A tora basal foi incorporada à estrutura após permanecer enterrada por dois meses, período necessário para o desenvolvimento de brotos [@Mira_Evette_2021], desempenhando dupla função,  propagação da espécie e barreira contra enxurradas. Para induzir brotamento e enraizamento, perfuraram-se os entrenós da tora basal, com preenchimento de água e posterior vedação, procedimento adotado para elevar a disponibilidade hídrica local e reduzir limitação inicial ao crescimento vegetativo. 

Em seguida, as toras horizontais foram instaladas e fixadas às verticais com arame recozido. Para aumentar a eficiência na retenção de sedimentos, foram adicionados sacos de ráfia preenchidos com solo local, ou sacos vazios fixados na face a montante das paliçadas, conforme [@nardin_et_al_2010] (Figura 4). A integridade estrutural do conjunto foi mantida por intervenções periódicas em ciclo de oito meses, com substituição de arames recozidos e renovação dos sacos de ráfia posicionados na face frontal, procedimento voltado a aumentar a retenção sem bloquear o escoamento e reduzir risco de colapso do depósito acumulado sob eventos de alta energia.

![Figura 4 -- Etapas de construção das paliçadas -- (a) Coleta de toras de bambu; (b) corte das toras; (c) coleta dos dados morfométricos das ravinas; (d) início de construção da paliçada; (e) amarrio das toras; (f) paliçadas prontas.](media/artigo/Fig_7v2.png){width="6.5in"}
...
Para complementar a análise, foram utilizados dados pluviométricos obtidos na estação meteorológica de Aracaju-SE, a fim de correlacionar os eventos de precipitação com as variações no movimento do solo. Nesse arranjo, a série pluviométrica é tratada como forçante climática regional representativa do regime sazonal e da recorrência de meses de alta energia, com uso coerente à escala temporal do monitoramento e às análises por agregação mensal adotadas neste trabalho.

Para dar suporte à leitura hidrossedimentológica do período monitorado em relação à variabilidade interanual e à sazonalidade, a série diária de 20 anos e o acumulado móvel de 30 dias foram utilizados como referência de severidade climática regional, preservando a coerência de escala entre a forçante pluviométrica e a resposta deposicional integrada em base mensal (Figura 5).

![Figura 5 -- Série de precipitação diária (novembro de 2005 a novembro de 2025) e acumulado móvel de 30 dias, usada na contextualização climatológica regional.](media/artigo/serie_precipitacao_20anos_pt.png){width="6.5in"}

## 2.6 Protocolo de monitoramento e condições de contorno
...
As análises estatísticas foram conduzidas em Python 3.9 com pandas, NumPy, SciPy e statsmodels, adotando nível de significância de 5% (α = 0,05) para testes de hipótese. A correlação de Pearson foi aplicada às séries mensais de precipitação e sedimentação incremental, com intervalo de confiança de 95% estimado por bootstrap (1000 replicações). A ANOVA de um fator foi utilizada para comparar médias entre segmentos, e a regressão linear simples reportou coeficiente de determinação (R²) como medida de capacidade explicativa. A análise de defasagem (lag) testou janelas simétricas de até ±6 meses para identificar resposta temporalmente deslocada entre forçante pluviométrica e deposição.

Ao todo, foram construídas quatro paliçadas na ravina estudada, identificadas como P1, P2, P3 e P4 na representação espacial e utilizadas como unidades experimentais para avaliar a retenção de sedimentos. As medições foram realizadas em três segmentos distintos da ravina: superior, intermediário e inferior, permitindo uma análise detalhada da eficiência das paliçadas em diferentes condições morfométricas e hidrológicas.


## 2.7 Eventos extremos e integração temporal

Para contextualização regional da severidade das chuvas, as curvas Intensidade--Duração--Frequência (IDF) foram representadas pela equação adotada proposta por @melo_et_al_2015:

| $\displaystyle i(t,T) = \frac{K\cdot T^a}{(t + b)^c}$ | (1) |
| --- | --- |

Na Equação 1, $i$ é a intensidade (mm/h), $t$ a duração (min), e $T$ o período de retorno (anos). Adotaram-se parâmetros $K = 1200$, $a = 0{,}20$, $b = 15$ e $c = 0{,}80$ para compor o envelope de referência utilizado na comparação com os eventos do período monitorado.
...
Como as leituras foram mensais, a comparação foi estruturada no regime hidrológico mensal, no qual a sequência de precipitações e a distribuição intra-mensal de intensidades são incorporadas ao total do mês e se manifestam na resposta deposicional como efeito integrado das condições antecedentes e da conectividade hidrossedimentológica.
...
| $\displaystyle C_r\;=\;\operatorname{clip}\Bigl(C_{ref}\,\bigl(\tfrac{VIB_{ref}}{VIB}\bigr)^{\alpha},\;C_{min},\;C_{max}\Bigr)$ | (2) |
| --- | --- |
...
A parametrização do coeficiente de escoamento superficial em função da velocidade de infiltração básica fundamenta-se no princípio físico estabelecido por @horton_1933 e @horton_1940, segundo o qual o escoamento superficial é gerado quando a intensidade de precipitação excede a capacidade de infiltração do solo. Solos com maior VIB apresentam maior capacidade de absorção da água precipitada, resultando em menor geração de escoamento superficial e, consequentemente, menor coeficiente de escoamento [@foster_meyer_onstad_1977]. 

A modulação da sedimentação pelo coeficiente de escoamento segue a abordagem de modelos processuais como EUROSEM [@morgan_et_al_1998] e WEPP [@nearing_et_al_1989], nos quais a capacidade de transporte de sedimentos é diretamente proporcional ao volume e energia do escoamento superficial. Esse acoplamento é coerente com a formulação da USLE/RUSLE [@wischmeier_smith_1978], que estabelece relações quantitativas entre escoamento e desprendimento de sedimentos.

A espessura incremental deposicional de base para cada limiar (P90/P95) foi obtida pela tradução empírica chuva--incremento, derivada da razão média por segmento entre sedimentação incremental (incremento fracionado, cm) e precipitação mensal (mm), conforme a Equação 3.


| $\displaystyle \Delta z_{base}(P_{xx})\;=\;\Bigl(\overline{\tfrac{\Delta z_{FRAC}}{P}}\Bigr)_{seg}\,\cdot\,P_{xx}$ | (3) |
| --- | --- |

Em seguida, a modulação hidrológica foi aplicada multiplicando-se $\Delta z_{base}$ por um fator relativo de escoamento, definido pela razão entre o coeficiente efetivo e o coeficiente de referência, conforme a Equação 4.


| $\displaystyle \Delta z_{eff}(P_{xx})\;=\;\Delta z_{base}(P_{xx})\cdot\Bigl(\tfrac{C_r}{C_{ref}}\Bigr)$ | (4) |
| --- | --- |
...
## 3.5 Eventos extremos, limiares operacionais e resposta deposicional

A variabilidade hidrológica foi sintetizada por quantis (Tabela 4), onde se vê os limiares operacionais, o tamanho amostral e a sedimentação média mensal por segmento.

**Tabela 4** -- Síntese das classes de intensidade de precipitação mensal, limiares operacionais, tamanho amostral e sedimentação média mensal por segmento.
...
O caráter episódico do processo erosivo foi corroborado pela distribuição assimétrica da deposição, na qual os percentis 90 e 95 da deposição incremental (0,0578 cm e 0,0853 cm) delimitaram a ocorrência de precipitações de alta magnitude. A concentração de 40,56% de todo o sedimento retido em quatro meses com deposição acima do percentil 95 sugere que a estabilização de ravinas em Plintossolos, sob as condições monitoradas, é fortemente influenciada por choques hidrossedimentológicos, e não apenas por deposição contínua [@Chen_2022].

A análise da série temporal acumulada (Figura 6) materializa essa dinâmica de "degraus", onde longos períodos de estabilidade ou baixa deposição são interrompidos por saltos abruptos de retenção associados aos eventos extremos, corroborando o modelo de preenchimento por pulsos. A sobreposição dos eventos de precipitação acumulada com a curva de sedimentação sugere que a resposta do sistema tende a não ser gradual, mas sim um reflexo direto da ativação de limiares de energia que mobilizam grandes volumes de sedimento em curtos intervalos de tempo.

![Figura 6 – Dinâmica temporal da sedimentação acumulada em resposta a eventos pluviométricos extremos. ](media/artigo/18_serie_eventos_extremos_ACUMULADA.png){width="6.5in"}

Essa desproporção entre frequência e magnitude, onde menos de 10% dos meses respondem por quase metade da retenção, indica um padrão no qual a estabilização de ravinas em Plintossolos é predominantemente governada por precipitações de alta energia. A contribuição relativa desses episódios de deposição extrema (acima do percentil 95 da deposição incremental) foi consistente entre os segmentos, representando 44,55% da deposição no segmento inferior, 39,55% no superior e 37,40% no intermediário.

A relação entre chuva extrema e deposição extrema não se mostrou linear. Observou-se que eventos de precipitação classificados como "Muito Alto" ou "Extremo" (> P90 da precipitação mensal no período monitorado) nem sempre geraram os maiores picos de deposição. A sedimentação média mensal na classe de chuva "Alta" (P75–P90) foi de 0,0669 cm, superior à registrada nas classes "Muito Alta" (0,0053 cm) e "Extrema" (0,0535 cm). Esse comportamento não-monotônico sugere a ocorrência de um limiar de saturação de transporte ou efeito de *bypass* [@Chen_2024], manifestando-se sob condições de fluxo torrencial extremo quando a energia cinética do escoamento supera a capacidade de decantação das paliçadas, mantendo partículas finas em suspensão e transportando-as para jusante, ou remobilizando depósitos prévios por turbulência localizada (*scouring*) [@frankl_et_al_2021].

A lenta absorção pode favorecer a saturação precoce do perfil e a conversão do excedente chuvoso em escoamento superficial de alta energia cinética, que mantém a fração fina em suspensão e, por turbulência localizada (*scouring*), inibe a deposição estável a montante das paliçadas. @frankl_et_al_2021 alertam para esse fenômeno, observando que a eficiência de *check dams* pode cair drasticamente em eventos de alta magnitude devido ao *scouring* e ao transporte de sedimentos em suspensão que ultrapassam a barreira sem decantar (*bypass*), limitando a eficácia de estruturas permeáveis durante picos de vazão.
...
![Figura 7 -- (a) Enquadramento regional (IDF) e (b) leitura temporal da precipitação no período monitorado, com identificação de eventos extremos (P95).](media/artigo/21a_curvas_idf_serie_temporal.png){width="6.5in"}
...
A análise de capacidade residual revelou que o sistema encontra-se no estágio inicial de preenchimento, com taxas de ocupação de 1,0% no segmento superior, 0,4% no intermediário e 1,4% no inferior (Figura 8a). Considerando as alturas úteis observadas de 50 cm (SUP), 76 cm (MED) e 36 cm (INF), a capacidade remanescente totaliza 49,5 cm, 75,7 cm e 35,5 cm, respectivamente. Essa baixa ocupação após dois anos de monitoramento é coerente com o regime episódico de sedimentação, no qual a retenção concentra-se em poucos eventos extremos ao longo do ano hidrológico [@parsons_et_al_2015].

A simulação espacial do preenchimento sedimentar (Figura 9) ilustra a evolução temporal da acumulação a montante das paliçadas em quatro estágios: 25%, 50%, 75% e 100% da capacidade máxima. As curvas de nível evidenciam a formação de cunhas deposicionais com decaimento longitudinal, características de barreiras permeáveis transversais ao fluxo [@xu_fu_he_2013].

A distribuição espacial mostra preenchimento preferencial nas zonas imediatamente a montante de cada estrutura, compatível com a redução da tensão cisalhante e o aumento do tempo de residência hidráulica nessa região [@wang_et_al_2021]. O estágio atual do sistema (<2% de ocupação) é ilustrado (Figura 9a). À medida que o preenchimento avança (Figuras 9b, 9c e 9d), observa-se a progressão da cunha deposicional até a crista, com redução do volume remanescente para acomodar novos aportes e aumento da probabilidade de extravasamento ou *bypass*.


![Figura 9 -- Simulação espacial da evolução temporal do preenchimento sedimentar nas paliçadas: (a) 25% da capacidade (~0,2-1,2 anos), (b) 50% (~0,4-2,4 anos), (c) 75% (~0,6-3,6 anos), e (d) saturação (100%, 0,8-4,8 anos).  ](media/artigo/fig_espacial_temporal_capacidade.png){width="6.5in"}

**Nota:** As cores representam os segmentos, SUP (coral, 50 cm), MED (turquesa, 76 cm), INF (verde, 36 cm). A simulação assume taxas de deposição constantes e não considera compactação ou remobilização. As curvas de nível indicam a espessura de sedimento (cm).

Ao considerar o limite geométrico de armazenamento sem extravasamento, a simulação de capacidade máxima representa o preenchimento da cunha deposicional até a altura útil de cada paliçada, incorporando as diferenças entre estruturas superior, intermediária e inferior e produzindo um cenário de reorganização morfológica coerente com o nivelamento parcial do leito por deposição controlada em série (Figura 10). O ortomosaico do domínio de referência é apresentado (Figura 10a), enquanto a leitura de tempo até saturação é apresentada nos painéis subsequentes (Figuras 10b, 10c e 10d) para um cenário médio e para os limiares P90 e P95, respectivamente, mantendo-se a restrição geométrica de armazenamento a montante das paliçadas. A parametrização da altura útil adotou 50 cm para a paliçada superior, 76 cm para as paliçadas intermediárias e 36 cm para a paliçada inferior, valores consistentes com o estoque retido reportado no período monitorado, e o produto deve ser interpretado como envelope geométrico de retenção potencial e não como previsão determinística de curto prazo.

![Figura 10 -- (a) Ortomosaico do recorte espacial da Ravina 1; (b) simulação espacial em regime de capacidade máxima no cenário médio, com rótulos em anos para saturação; (c) simulação espacial em regime de capacidade máxima para o limiar P90; (d) simulação espacial em regime de capacidade máxima para o limiar P95.](media/artigo/fig_sim_anos.png){width="6.5in"}

À medida que a cunha deposicional se aproxima da crista no cenário de capacidade máxima (Figura 10), reduz-se o volume remanescente para acomodar novos aportes e aumenta a probabilidade de passagem preferencial de sedimentos finos em suspensão por *bypass*, além de intensificar a redistribuição do depósito por escavação localizada e reequilíbrio do leito sob variações de tensão cisalhante. Nesse estágio, a métrica volumétrica sintetiza a proximidade do sistema ao extravasamento e conecta desempenho geomorfológico e tomada de decisão, orientando a priorização de manutenção quando a capacidade remanescente se torna pequena.
...
A dinâmica deposicional mostrou caráter não linear e dominado por precipitações, nos quais eventos de maior energia hidrológica geram incrementos desproporcionais de aporte e redistribuição de sedimentos. Sob essas condições, a eficiência de retenção tende a reduzir-se quando o regime hidráulico se aproxima do torrencial, cenário no qual aumenta a probabilidade de *bypass* de frações finas em suspensão e de remobilização local por turbulência e erosão de fundo, o que reforça a necessidade de interpretar o desempenho como resposta condicionada por limiares e não como função direta do total de chuva.
