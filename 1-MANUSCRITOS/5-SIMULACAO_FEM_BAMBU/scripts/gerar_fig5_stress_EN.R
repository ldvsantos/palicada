## ---------------------------------------------------------------
## Fig. 5 — Vertical stress distribution (σ_b and τ_s) (EN version)
## MED segment / P95 / pessimistic
## Regression with model selection (linear vs quadratic, ANOVA)
## ---------------------------------------------------------------
...
df <- read.csv(file.path(data_dir, "fig5_stress_data.csv"))
df$time_yr <- factor(df$time_yr, levels = c(0, 5, 10),
                     labels = c("t = 0 yr", "t = 5 yr", "t = 10 yr"))

# ---- Model selection per series (ANOVA: linear vs quadratic) ----
pred_list <- list()
annot_list <- list()
...
shapes <- c("t = 0 yr" = 16, "t = 5 yr" = 15, "t = 10 yr" = 17)
...
p1 <- ggplot() +
  ...
  add_scales("sigma") +
  labs(x = expression(sigma[b]~" (MPa)"), y = "Height z (m)",
       tag = "(a)") +
  ggtitle("Bending stress") +
  theme_academic
...
cat("Fig_5_stress_height (EN) saved to:", fig_dir, "\n")
