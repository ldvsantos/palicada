## ---------------------------------------------------------------
## Fig. 3 — Temporal evolution of max Tsai-Hill index (FI_max)
## MED segment / 3 degradation scenarios × 2 hydrological scenarios
...
df <- read.csv(file.path(data_dir, "fig3_fi_evolution_data.csv"))
...
# ---- Fit model for each degradation × hydro combination ----
...
# ---- Legend labels (EN) ----
deg_labels_en <- c(optimistic = "Optimistic", baseline = "Reference",
...
make_label <- function(deg, hyd) {
  key <- paste(deg, hyd, sep = "_")
  fit <- fits[[key]]
  sprintf("%s · %s (R² = %.3f)", deg_labels_en[deg], hyd, fit$r2)
}
...
p <- ggplot() +
  ...
  geom_hline(yintercept = 1.0, colour = "black", linetype = "dotted",
             linewidth = 0.6) +
  annotate("text", x = 0.3, y = 1.05, label = "FI = 1 (failure)",
