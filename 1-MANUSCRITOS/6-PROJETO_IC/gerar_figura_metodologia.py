...
for (xcp, tipo) in positions:
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        ...
        alpha=0.75, zorder=4))
    # CP interior (retângulo branco com hachura)
    inner_w, inner_h = 0.7, 1.05
    ax.add_patch(plt.Rectangle((xcp - inner_w/2, cp_y - inner_h/2),
                                inner_w, inner_h,
                                color="white", alpha=0.6, zorder=5))
    for dy in np.arange(0.1, inner_h, 0.25):
        ax.plot([xcp - inner_w/2, xcp + inner_w/2],
                [cp_y - inner_h/2 + dy]*2,
                color=col, lw=1, alpha=0.9, zorder=6)
